...
test("seasonal queue priority follows September through December", () => {
  assert.equal(priority("Patriot Day remembrance shirt"), 10);
  assert.equal(priority("Fall leaves graphic tee"), 15);
  assert.equal(priority("Halloween pumpkin shirt"), 20);
  assert.equal(priority("Thanksgiving autumn turkey shirt"), 30);
  assert.equal(priority("Christmas winter ornament"), 40);
...
test("specific events take precedence over broad seasonal words", () => {
  assert.equal(priority("Fall Thanksgiving family shirt"), 30);
  assert.equal(priority("Autumn Halloween pumpkin shirt"), 20);
  assert.equal(priority("Thanksgiving turkey trot with pumpkin accents"), 30);
  assert.equal(priority("Teacher Halloween costume shirt"), 20);
...
test("queue rows are ordered by event month and then creation date", () => {
  const rows = sortQueueRowsForPublishing([
    ...
    { title: "September teacher shirt older", description: null, created_at: "2026-01-01T00:00:00Z" },
    { title: "Thanksgiving shirt", description: null, created_at: "2026-01-01T00:00:00Z" }
  ]);

  assert.deepEqual(rows.map((row) => row.title), [
    "September teacher shirt older",
